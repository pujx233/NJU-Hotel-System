        ...
        <a-menu v-model="current" mode="horizontal" theme="light">
            ...
            <a-menu-item key="3" @click="selectMenu" v-if="userInfo.userType=='HotelManager'">
                <router-link :to="{ name: 'manageHotel'}">
                     <a-icon type="form" />酒店管理
                </router-link>
            </a-menu-item>
            <a-menu-item key="4" @click="selectMenu" v-if="userInfo.userType=='Administrator'">
                <router-link :to="{ name: 'manageUser'}">
                     <a-icon type="user" />账户管理
                </router-link>
            </a-menu-item>
            <a-menu-item key="5" @click="selectMenu" v-if="userInfo.userType=='Administrator'">
                <router-link :to="{ name: 'addHotel'}">
                    <a-icon type="plus" />添加酒店
                </router-link>
            </a-menu-item>
            <a-menu-item key="6" @click="selectMenu" v-if="userInfo.userType=='Marketer'">
                <router-link :to="{ name: 'addCredit'}">
                    <a-icon type="user" />信用充值
                </router-link>
            </a-menu-item>
            <a-menu-item key="7" @click="selectMenu" v-if="userInfo.userType=='Marketer'">
                <router-link :to="{ name: 'webCoupon'}">
                    <a-icon type="money-collect" />网站促销策略
                </router-link>
            </a-menu-item>
            <a-menu-item key="8" @click="selectMenu" v-if="userInfo.userType=='Marketer'">
                <router-link :to="{ name: 'abnormalOrder'}">
                    <a-icon type="question-circle" />异常订单
                </router-link>
            </a-menu-item>
        </a-menu>
        <div class="logout">
...
export default {
    ...
    mounted() {
        if (this.$route.name == 'hotelList' || this.$route.name == 'hotelDetail') {
            ...
            this.current = ['4']
        }else if(this.$route.name == 'addHotel'){
            this.current = ['5']
        }else if(this.$route.name == 'addCredit'){
            this.current = ['6']
        }else if(this.$route.name == 'webCoupon'){
            this.current = ['7']
        }else if(this.$route.name == 'abnormalOrder'){
            this.current = ['8']
        }
    },
    methods: {
